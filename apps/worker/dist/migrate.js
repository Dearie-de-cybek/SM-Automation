...
var MIGRATION_LOCK_ID = 8273423517001;
var FILE_RE = /^(\d+)[_-](.+)\.sql$/;
...
async function findMigrationsDir(startDir = process.cwd()) {
...
async function runMigrations(sql, options = {}) {
...
var SECRET_KEY_RE = /(token|secret|password|passwd|credential|authorization|auth|cookie|signature|api[-_]?key|access[-_]?key|private[-_]?key|verifier|session)/i;
...
function createLogger(options = {}) {
  ...
  const write = options.write ?? ((line, lineLevel) => {
    if (lineLevel === "error" || lineLevel === "warn") console.error(line);
    else console.log(line);
  });
  const emit = (lineLevel, message, fields) => {
