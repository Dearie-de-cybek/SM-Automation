...
const CLIENT_COLUMNS = `
  ...
  c.meta_token_enc IS NOT NULL AS meta_connected, c.meta_connected_at, c.timezone, c.active, c.created_at`;
...
export async function listClientsForAdmin(): Promise<AdminClientRow[]> {
  const sql = db();
  return sql<AdminClientRow[]>`
    SELECT ${sql.unsafe(CLIENT_COLUMNS)},
           count(p.id)::int AS posts_total,
           count(p.id) FILTER (WHERE p.status = 'pending_approval')::int AS posts_awaiting,
           count(p.id) FILTER (WHERE p.status IN ('published', 'partially_published'))::int AS posts_published,
